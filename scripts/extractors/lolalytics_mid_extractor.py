...
LANE_QUERY = "middle"
RANK = "emerald_plus"
RANK_QUERY = "emerald_plus"
OUTPUT_PATH = PROJECT_ROOT / "data" / "external" / "lolalytics_mid_pickrate_mainrate.csv"
...
POPULATION_SCOPE_PICKRATE = "global_emerald_plus_ranked_solo_duo_mid"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--patch", default=None, help="Historical patch like 16.6")
    parser.add_argument(
        "--output",
...
def build_tierlist_url(patch: str | None) -> str:
    query = {
        "lane": LANE_QUERY,
        "tier": RANK_QUERY,
    }
    if patch:
        query["patch"] = patch
    return f"{BASE_URL}/lol/tierlist/?{urlencode(query)}"
...
def get_champion_rows(page: Page, tierlist_url: str) -> tuple[list[ChampionRow], str]:
...
def main() -> None:
    ...
    tierlist_url = build_tierlist_url(args.patch)
    ...
    try:
        ...
        champions, patch = get_champion_rows(list_page, tierlist_url)
